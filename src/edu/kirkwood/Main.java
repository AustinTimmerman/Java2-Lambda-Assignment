...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Predicate<Widget> zeroOrGreater = widgetInput -> widgetInput.getValue() >= 0;
        ...
        UnaryOperator<Widget> nameToLower = widgetInput -> {
            widgetInput.setName(widgetInput.getName().toLowerCase());
            return widgetInput;
        };

        // STEP 8: Create a Consumer called "print" to print each Widget as a string on a new line using the .toString() method.
        // This code should be written on a single line.
        Consumer<Widget> print = widgetInput -> System.out.println(widgetInput.toString());


        for(int i = 0; i < 20; i++) {
            // STEP 1: Create a Supplier object called "widgetCreator" using a method reference.
            Supplier<Widget> widgetCreator = () -> new Widget();
